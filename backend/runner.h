...
{
    public:
        RunCommand(const QString& command, const QStringList& args,
const QDir& cwd)
        : command_(command)
        , arguments_(args)
        , cwd_(cwd)
...
class Process {
  public:
    ...
    Process(const std::vector<std::shared_ptr<ICommand>>& commands,
            bool showDialog);
...
class backend_EXPORT ActionRunner : public QObject {
    Q_OBJECT
