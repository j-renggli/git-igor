...
Backend& Backend::instance() { return s_backend; }

Backend::Backend() : resourcesPath_(SHARED_DATA_FOLDER) {}
...
bool Backend::initialise(QObject* mainWin) {
    ...
    htmlPath_ = resourcesPath_;
    ok = ok && htmlPath_.cd("res/js");
    ...
    if (ok) {
        ...
        connect(&runner_, &ActionRunner::notifyStartWork, this,
                &Backend::workStarted);
        connect(&runner_, &ActionRunner::notifyStopWork, this,
                &Backend::workEnded);
        connect(&runner_, &ActionRunner::notifyOutput, this,
                &Backend::workOutput);
    }
...
void Backend::workStarted(const QString& command) {
} // std::cout << command.toLatin1().data() << std::endl; }
